...
library(tibble)
## Load the data
mean_absg_habitats <- readRDS("mixed_model/save_samples/mean_absg_habitats.rds")
sd_absg_habitats <- readRDS("mixed_model/save_samples/sd_absg_habitats.rds")

## JAGS data
data_list <- list(absg = as.matrix(mean_absg_habitats),
                  sd = as.matrix(sd_absg_habitats),
                  habitats = nrow(mean_absg_habitats),
                  years = ncol(mean_absg_habitats),
                  t = 1:ncol(mean_absg_habitats))
...
cat(
  ...
  for(hab in 1:habitats){
    for(year in 1:years){
      ...
      absg[hab,year] ~ dnorm(mu[hab,year], tau)
      mu[hab,year] <- beta0[hab] + beta1[hab] * t[year] + error[hab,year]

    }
  }

  }", fill = TRUE, file = "mixed_model/models/absg_hab.txt")
...
## Fit the model
model <- jags(model.file = "mixed_model/models/absg_hab.txt",
...
saveRDS(model, "mixed_model/outputs/perhabitat_absg_output.rds")
