...
library(jagsUI)
## Load the data
mean_r_overall <- readRDS("mixed_model/save_samples/mean_rr_overall.rds")
sd_r_overall <- readRDS("mixed_model/save_samples/sd_rr_overall.rds")

## JAGS data
data_list <- list(rr = as.matrix(mean_r_overall),
                  sd = as.matrix(sd_r_overall),
                  routes = nrow(mean_r_overall),
                  years = ncol(mean_r_overall),
                  t = 1:ncol(mean_r_overall))
## JAGS model
cat(
  "model{
  ...
  for(r in 1:routes){
    for(year in 1:years){
      ...
      rr[r,year] ~ dnorm(mu[r,year], tau)
      mu[r,year] = beta0[r] + beta1[r] * t[year] + error[r,year]

    }
  }

  }", fill = TRUE, file = "mixed_model/models/r_overall.txt")
...
## Fit the model
model <- jags(model.file = "mixed_model/models/r_overall.txt",
...
saveRDS(model, "mixed_model/outputs/overall_r_output.rds")
