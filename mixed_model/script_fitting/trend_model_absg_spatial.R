...
library(jagsUI)
## Load the data
mean_absg_overall <- readRDS("mixed_model/save_samples/mean_absg_overall.rds")
sd_absg_overall <- readRDS("mixed_model/save_samples/sd_absg_overall.rds")

## JAGS data
data_list <- list(absg = as.matrix(mean_absg_overall),
                  sd = as.matrix(sd_absg_overall),
                  routes = nrow(mean_absg_overall),
                  years = ncol(mean_absg_overall),
                  t = 1:ncol(mean_absg_overall))
...
cat(
  ...
  for(r in 1:routes){
    for(year in 1:years){
      ...
      absg[r,year] ~ dnorm(mu[r,year], tau)
      mu[r,year] = beta0[r] + beta1[r] * t[year] + error[r,year]

    }
  }

  }", fill = TRUE, file = "mixed_model/models/absg_overall.txt")
...
## Fit the model
model <- jags(model.file = "mixed_model/models/absg_overall.txt",
...
saveRDS(model, "mixed_model/outputs/overall_absg_output.rds")
