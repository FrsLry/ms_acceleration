...
library(tibble)
## Load the data
mean_r_habitats <- readRDS("mixed_model/save_samples/mean_rr_habitats.rds")
sd_r_habitats <- readRDS("mixed_model/save_samples/sd_rr_habitats.rds")

## JAGS data
data_list <- list(r = as.matrix(mean_r_habitats),
                  sd = as.matrix(sd_r_habitats),
                  habitats = nrow(mean_r_habitats),
                  years = ncol(mean_r_habitats),
                  t = 1:ncol(mean_r_habitats))
...
cat(
  ...
  for(hab in 1:habitats){
    for(year in 1:years){
      ...
      r[hab,year] ~ dnorm(mu[hab,year], tau)
      mu[hab,year] <- beta0[hab] + beta1[hab] * t[year] + error[hab,year]

    }
  }

  }", fill = TRUE, file = "mixed_model/models/r_hab.txt")
...
## Fit the model
model <- jags(model.file = "mixed_model/models/r_hab.txt",
...
saveRDS(model, "mixed_model/outputs/perhabitat_r_output.rds")
