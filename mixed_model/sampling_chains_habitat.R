...
## in order to propagate the uncertainties at the habitat level
library(dplyr)
library(tidyr)
...
N_habitat <- array(0, dim = c(length(unique(family$Cornell_Habitat)), 35, s))
R_habitat <- array(0, dim = c(length(unique(family$Cornell_Habitat)), 34, s))
L_habitat <- array(0, dim = c(length(unique(family$Cornell_Habitat)), 34, s))

## Give name of the first dimension as the habitat
dimnames(N_habitat) <- list(unique(family$Cornell_Habitat))
dimnames(R_habitat) <- list(unique(family$Cornell_Habitat))
dimnames(L_habitat) <- list(unique(family$Cornell_Habitat))
...
mean_sd_N_hab <- data.frame(habitat = as.character(), year = as.numeric(), mean_N = as.numeric(), sd_N = as.numeric())
...
for(hab in unique(family$Cornell_Habitat)){

  tmp_hab <- family[family$Cornell_Habitat == hab,]

  for(species in tmp_hab$English_Common_Name){
    ...
    N_habitat[which(hab == unique(family$Cornell_Habitat)),,] <- N_habitat[which(hab == unique(family$Cornell_Habitat)),,] + t(N_sp)
    R_habitat[which(hab == unique(family$Cornell_Habitat)),,] <- R_habitat[which(hab == unique(family$Cornell_Habitat)),,] + t(R_sp)
    L_habitat[which(hab == unique(family$Cornell_Habitat)),,] <- L_habitat[which(hab == unique(family$Cornell_Habitat)),,] + t(L_sp)
...
saveRDS(N_habitat, "mixed_model/save_samples/N_habitat.rds")
saveRDS(R_habitat, "mixed_model/save_samples/R_habitat.rds")
saveRDS(L_habitat, "mixed_model/save_samples/L_habitat.rds")
...
mean_N_habitats <- apply(N_habitats, c(1,2), mean)
sd_N_habitats <- apply(N_habitats, c(1,2), sd)
